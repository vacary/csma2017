...
\begin{table}
  ...
  \begin{tabular}{ll}
    ...
    $\sf NSN-AC-NLS-HYBRID$ & Méthode $\sf NSN-AC-NLS$ avec  un pré--conditionnement par $100$ itérations de NSGS-AC \\
    $\sf NSGS-AC$ & Méthode de Gauss Seidel avec NSN-AC-NLS pour méthode locale\\
    $\sf NSGS-FP-VI-UPK$ & Méthode de Gauss Seidel avec itération de points fixe de $F_\nat(r)-r$\\
  \end{tabular}
...
\begin{figure}
  ...
  \caption{Comparaison entre les méthodes itérative de type projection et les méthodes de Newton non--lisses pour différentes précisions}
...
intégration du non-linéaire de comportement ou géométrique
